...
use font::{Fonts, font_from_style,NORMAL_STYLE, DISPLAY_STYLE};
...
impl View for Intermission {
    ...
    fn render(&self, fb: &mut Framebuffer, fonts: &mut Fonts) {
        ...
        let dy = (self.rect.height() as i32)-x_height ;

        let r = Rectangle::new(Point {x: 0, y: dy-3*x_height}, Point {x: self.rect.width() as i32, y: dy + x_height});

        fb.draw_rectangle(&r, scheme[0]);
